...
import numbers
from tool.models.archs.Uformer_arch1 import window_partition,window_reverse
...
class LinearProjection(nn.Module):
    ...
    def flops(self, q_L, kv_L=None): 
        ...
        return flops 
def re_window_partition(x, win_size, dilation_rate=1):
    B, H, W, C = x.shape
    if dilation_rate !=1:
...
class WindowAttention(nn.Module):
    def __init__(self, dim, win_size,num_heads, token_projection='conv', qkv_bias=True, qk_scale=None, attn_drop=0., proj_drop=0.):
...
class Attention_win(nn.Module):
    def __init__(self, dim, num_heads, bias,win_size=8,qkv_bias=True,token_projection='linear'):
    ...
    def forward(self, x):
        ...
        x = self.output_resize(shifted_x)+short_cut # B C H W .contiguous()
...
class TransformerBlock(nn.Module):
    def __init__(self, dim, num_heads, ffn_expansion_factor, bias, LayerNorm_type,win_size):
        ...
        self.attn = Attention_win(dim=dim, num_heads=num_heads, bias=bias,win_size=win_size)
