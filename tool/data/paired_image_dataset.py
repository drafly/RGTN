...
from tool.data.data_util import (paired_paths_from_folder,
                                    paired_DP_paths_from_folder,
                                    paired_paths_from_lmdb,
                                    paired_paths_from_meta_info_file)
from tool.data.transforms import augment, paired_random_crop, paired_random_crop_DP, random_augmentation,paired_random_crop_mask
from tool.utils import FileClient, imfrombytes, img2tensor, padding, padding_DP, imfrombytesDP,padding_withmask
...
class Dataset_PairedImage_test(data.Dataset):
    ...
    def __init__(self, opt):
        super(Dataset_PairedImage_test, self).__init__()
        self.opt = opt
...
class Dataset_maskImage(data.Dataset):
    """Paired image dataset for image restoration.
    ...
    def __init__(self, opt):
        super(Dataset_maskImage, self).__init__()
        self.opt = opt
        # file client (io backend)
        self.file_client = None
        self.io_backend_opt = opt['io_backend']
    ...
    def __getitem__(self, index):
        ...
        mask_path = self.paths[index]['lq_path'].replace('input','sub50')
        ...
        if self.opt['phase'] == 'train':
            gt_size = self.opt['gt_size']
            # padding
            img_gt, img_lq,img_mask = padding_withmask(img_gt, img_lq,img_mask, gt_size)

            # random crop
            img_gt, img_lq,img_mask = paired_random_crop_mask(img_gt, img_lq,img_mask, gt_size, scale,
                                                gt_path)
